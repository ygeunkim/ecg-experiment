...
- \textcolor{red}{Does caffeine affect ECG or \it{heart rate}?}
- Caffeine: \emoji{coffee} *coffee*
- Output: \emoji{watch} Average heart rate (BPM)
...
\begin{enumerate}[A]
  \item House blend (74.5 mg) to `1`
  \item Water (0 mg) to `4`
  \item Sumatra (54.5 mg) to `2`
  \item Decaf espresso roast (3 mg) to `3`
\end{enumerate}
...
From previous Table \@ref(tab:obsouttab),

$$\hat\tau(k) = \ybar(k) - \ybar(\cdot)$$

is the unbiased estimator of $\tau(k) = \overline{Y}_{\cdot \cdot}(k) - \overline{Y}_{\cdot \cdot}(\cdot)$.
...
ecg_diff %>% 
  ...
  kable(
    format = kable_format,
    escape = FALSE,
    caption = "Unbiased Estimation: $\\hat\\tau(k)$",
...
- Here p-value is `r ecg_aov$p.value[3]`
...
- Other samples: Fix arbitrary levels
    - Re-define the levels of each factor
- Re-measure (for Unit `8`)
    - Figure \@ref(fig:hbrand): Change $y_{24} = `r ecg_diff$diff[8]`$ to `r new_val`
    - P-value becomes `r hb_pval` \emoji{partying-face}

```{r hbrand, echo=FALSE, fig.cap="If (2,4) were the different value \\emoji{thinking}"}
